# This exercise is very similar to the previous one. The only difference is that we will get better overview of how the
# neural network is learning. To do this we will split out input data into learning set and test set. We will perform
# all the learning using the first set of data. Of course as the number of epochs increases, so does the accuracy on the
# learning set (as the cost function on this set is minimized by the learning process). But every now and then we will
# check the accuracy on the test set - data that the network hasn't seen during the learning phase. Ideally the
# accuracy on the learning set should increase along with the accuracy on the test set, as this means the network
# has "learned" something general that can be applied on a data that was not seen.
# At the end we will draw a little chart to check how the accuracy was changing over time for both sets.
import argparse
...
# Convert the aswers vector to a sparse matrix (refer to 1_nn_training_example.py for a more detailed comment)
Y_sparse = sparse.csr_matrix((np.ones(numSamples), Y_data.reshape(numSamples), range(numSamples+1))).toarray()
Y_test_sparse = sparse.csr_matrix((np.ones(numTestSamples), Y_test_data.reshape(numTestSamples),
...
for epoch in xrange(args.epochs):
    ...
    if not epoch % 5:
        train_accuracy = sess.run([accuracy], feed_dict={x: X_data, y: Y_sparse})
        test_accuracy = sess.run([accuracy], feed_dict={x: X_test_data, y: Y_test_sparse})
